  ...
  def main(args: Array[String]): ProvInfo = {
    ...
    try {
      ...
      val types = trips.map { s => 
        val speed = s._2
        if (_root_.refactor.MonitorAttacher.provWrapper(speed > 40, q"$speed > 40", (List[Any](speed), List[Any]()), 0)) {
          ("car", speed)
        } else if (_root_.refactor.MonitorAttacher.provWrapper(speed > 15, q"$speed > 15", (List[Any](speed), List[Any](speed)), 1)) {
          ("public", speed)
        } else {
          ("onfoot", speed)
        }
