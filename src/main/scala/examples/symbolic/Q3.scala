...
//    item.take(10).foreach(println)
    ...
    val safety = date_dim.filter{ row => try { row(8).toInt; true } catch { case _: Throwable => false}}
    val filter1 = safety.filter(row => _root_.monitoring.Monitors.monitorPredicateSymEx(row(8).toInt/*d_moy*/ == MONTH, (List(), List()),0, expressionAccumulator))
    ...
    val filter2 = item.filter(row => _root_.monitoring.Monitors.monitorPredicateSymEx(row(13).toInt /*i_manufact_id*/ == MANUFACT, (List(), List()), 1, expressionAccumulator)) // and item.i_manufact_id = [MANUFACT]
